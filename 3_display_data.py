...
dt_L3Yr=(pd.to_datetime(trade_date_max)+ timedelta(days=-1080)).strftime('%Y%m%d')
...
st.write(df_sum_1[['index_code','名称','close','3年分位数_close','pe_ttm','3年分位数_pe_ttm','pb','3年分位数_pb']]\
         .style.hide_index().format({"close":"{:.2f}","3年分位数_close":"{:.2f}","pe_ttm":"{:.2f}","3年分位数_pe_ttm":"{:.2f}","pb":"{:.2f}","3年分位数_pb":"{:.2f}"}).to_html()\
...
dt_start =(pd.to_datetime(trade_date_max)+ timedelta(days=-1080))
...
df_weekly_metric_stock_3=df_weekly_metric_stock_2[df_weekly_metric_stock_2['con_code'].isin(top_stock_list)]
